    ...
    <span v-if="info.previous" @click="goToPage(prePage)" class="page-link">
        <button class="w-8 h-8 rounded mx-1 my-1 bg-gray-300">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mx-auto" viewBox="0 0 20 20" fill="currentColor">
                <path fill-rule="evenodd" d="M12.707 14.707a1 1 0 010-1.414L9.414 10l3.293-3.293a1 1 0 00-1.414-1.414l-4 4a1 1 0 000 1.414l4 4a1 1 0 001.414 0z" clip-rule="evenodd"></path>
            </svg>
        </button>
    </span>

    <a v-for="page in pages" :key="page" class=page-link>
        <button v-if="page==='...' " class="w-8 h-8 rounded mx-1 my-1 bg-gray-300 text-white">{{ page }}</button>
        <button v-else-if="page===current " class="w-8 h-8 rounded mx-1 my-1 bg-blue-500 text-white">{{ page }}</button>
        <button v-else @click="goToPage(page)" class="w-8 h-8 rounded mx-1 my-1 bg-gray-300 text-white">{{ page }}</button>
    </a>
